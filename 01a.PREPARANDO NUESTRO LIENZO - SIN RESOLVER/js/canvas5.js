...
/************propiedades del objeto juego 
             ...
             var juego={
                ...
                tiempo: function(){
                    ...
                    if(datos.izquierda==true){ jugador.movimiento_x = -jugador.velocidad; jugador.movimiento_y = 0; }
                    if(datos.derecha==true){ jugador.movimiento_x = jugador.velocidad; jugador.movimiento_y = 0;}
...
//NO COLICION DE IZQUIERDA A DERECHA

if(jugador.x2 < bloques[i].x1){return false;}


//NO COLICION DE DERECHA A IZQUIERDA

if(jugador.x1 > bloques[i].x2){return false;}

//NO COLICION DE ARRIBA A ABAJO

if(jugador.y2 < bloques[i].y1){return false;}

//NO COLICION DE ABAJO A ARRIBA
...
if(jugador.y1 > bloques[i].y2){return false;}
 ...
 coliciones();
 //colicion de dercha a izquierda
if(coliciones() && jugador.x2 < bloques[i].x1 + jugador.movimiento_x){
...
//coliciones de izquierda a derecha

if(coliciones () && jugador.x1 - jugador.movimiento_x > bloques[i].x2){
    jugador.movimiento_x=0;
   jugador.x=280;
   jugador.y=55;
...
//COLICIONES DE ARRIBA HACIA ABAJO
if(coliciones () && jugador.y2 < bloques[i].y1 + jugador.movimiento_y){
...
//COLICIONES DE ABAJO HACIA ARRIBA
if(coliciones () && jugador.y1  - jugador.movimiento_y > bloques[i].y2 ){
 jugador.movimiento_y=0;
   jugador.x=280;
   jugador.y=55;
   datos.segundos=0;
...
if(jugador.y2>395 && jugador.x2<280){
    alert("tu tiempo es de "+ datos.segundos);
   location.reload();
}
                    ...
                    if(datos.arriba==true){jugador.movimiento_y=-jugador.velocidad;jugador.movimiento_x = 0;} 
                    if(datos.abajo==true){jugador.movimiento_y=jugador.velocidad;jugador.movimiento_x = 0;}
                    
                    if(!datos.arriba && !datos.abajo){jugador.movimiento_y = 0; }

                    console.log("jugador  izquierda", jugador.movimiento_x);
...
/**
var jugador={} con las llaves haci declaradas se da entender 
que ya es un objeto */
...
/*var cre=0;
while(cre<bloques.length){
    ctx.fillRect(bloques[cre].x,bloques[cre].y,bloques[cre].ancho,bloques[cre].alto); 
  cre++;
  if(cre==bloques.length){break;}
}*/